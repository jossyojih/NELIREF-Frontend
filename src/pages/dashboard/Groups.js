    ...
    setMyGroups(
      data?.filter(
        (item) =>
          item?.privacy === 'public' ||
          item.members.some(
            (member) => member.user === user._id && member.status === 'approved'
          )
      )
    )
  ...
  return (
    <Wrapper>
      ...
      <article>
        ...
        <section>
          {activeTab === 'Photos' && (
            <AllGroups groups={data} isError={isError} isPending={isPending} />
          )}
          {activeTab === 'my-groups' && <MyGroups groups={myGroups} />}
